...
class SearchBar extends Component {
    ...
    render() {
        let getSingleChatBody = (matches) => {
            return matches.map(match => {
                return (
                    <div key={match.id}>
                        <div className={classnames('contact_item', {'on': this.state.active == match.id})}
                             onMouseEnter={e => this.setState({active: match.id})}
                             onClick={e => this.startChat(match.name, ChatType.CHAT)}>
        ...
        let getRoomChatBody = (matches) => {
            return matches.map(match => {
                return (
                    <div key={match.id}>
                        <div className={classnames('contact_item', {'on': this.state.active == match.id})}
                             onMouseEnter={e => this.setState({active: match.id})}
                             onClick={e => this.startChat(match.id, ChatType.GROUP_CHAT)}>
                            <div className="avatar">
                                <img className="img lazy" src="img/default.jpg"/>
                            </div>
                            <div className="info">
                                <h4 className="nickname ">{match.name}</h4>
                            </div>
                        </div>
                    </div>
                )
                            ...
                            <div className="scroll-wrapper contacts scrollbar-dynamic" style={{position: 'relative', 'height': '450px'}}>
                                <div className="contacts scrollbar-dynamic scroll-content scroll-scrolly_visible">
                                    <div>
                                        ...
                                        {
                                            this.state.matchPatients.length != 0 && ( <div>
                                                <h4 className="contact_title first">好友</h4>
                                            </div>)
                                        }
                                        {
                                            getSingleChatBody(this.state.matchPatients)
                                        }

                                        {
                                            this.state.matchRooms.length != 0 && ( <div>
                                                <h4 className="contact_title first">群组</h4>
                                            </div>)
                                        }
                                        {
                                            getRoomChatBody(this.state.matchRooms)
                                        }

                                        {
                                            this.state.matchDoctors.length != 0 && ( <div>
                                                <h4 className="contact_title first">医生</h4>
                                            </div>)
                                        }
                                        {
                                            getSingleChatBody(this.state.matchDoctors)
                                        }
